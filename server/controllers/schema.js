...
const getSchema = async (req, res) => {
    ...
    console.log('PostgreSQL Schema Extractor API');
    console.log('='.repeat(60));
    ...
    try {
        ...
        for (const e of enums) {
            // Ensure values is always an array (pg driver may return it differently)
            schema.enums[e.enum_name] = Array.isArray(e.values) ? e.values : [e.values];
        }
        ...
        for (const table of tables) {
            ...
            for (const u of unique) {
                schema.tables[table].unique_constraints.push({
                    ...
                    columns: Array.isArray(u.columns) ? u.columns : [u.columns]
                });
            }
        ...
        const jsonPath = path.join(schemasDir, 'schema.json');
        fs.writeFileSync(jsonPath, JSON.stringify(schema, null, 2));
        console.log(`\n✓ Saved JSON schema to: ${jsonPath}`);

        console.log('\n' + '='.repeat(60));
        console.log('Done! Schema extracted and saved.');
        console.log('='.repeat(60));
        ...
        res.status(500).json({ error: 'Failed to extract schema', details: error.message });
    } finally {
        client.release();
    }
};
...
function printSchemaSummary(schema) {
    ...
    if (Object.keys(schema.enums).length > 0) {
        ...
        for (const [name, values] of Object.entries(schema.enums)) {
            ...
            console.log(`  • ${name}: ${valuesArray.join(', ')}`);
        }
    }
